...
export function NavDropdown({ label, items, textColor, hoverColor }: NavDropdownProps) {
          ...
          <div className="min-w-[280px] max-w-[320px] max-h-[calc(100vh-120px)] overflow-y-auto overflow-x-hidden rounded-lg bg-white shadow-2xl border border-slate-200 py-2">
            {items.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="block px-4 py-2.5 text-slate-900 hover:bg-slate-50 transition-colors"
